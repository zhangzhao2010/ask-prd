...
- **描述**: 手动创建数据同步任务，异步处理文档转换和向量化

### 重要说明
- 此接口需要**用户手动触发**，不会在文档上传后自动执行
- 同步任务处理所有状态为`uploaded`的文档
- 处理流程包括：Marker转换、Claude Vision图片理解、文本分块、向量化、存储到S3和OpenSearch
...
1. **手动触发**: 同步任务不会在文档上传后自动创建，需要用户手动点击"同步数据"按钮触发
2. **并发限制**: 同一知识库同时只能运行一个同步任务
3. **任务时长**: 大文档处理可能需要数分钟
4. **进度查询**: 前端通过手动刷新按钮查询进度，不建议使用自动轮询（避免不必要的API请求）
5. **失败重试**: 部分失败时可使用retry-failed接口重试失败文档
6. **任务取消**: 取消任务会停止后续文档处理，已处理的文档不会回滚
7. **进度精度**: progress是估算值，可能不完全线性增长
